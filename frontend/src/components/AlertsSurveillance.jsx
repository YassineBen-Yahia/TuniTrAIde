...
const AlertsSurveillance = () => {
  ...
  const handleGetExplanation = async (alert) => {
    ...
    try {
      // Try to get AI explanation via chat API
      const response = await ApiService.sendChatMessage(
        `Explain this market alert for ${alert.symbol} (${alert.name}): "${alert.message}". What could be the reasons behind this movement and what should investors consider?`,
        null
      );
      setExplanation(response.response || response.message || 'Unable to generate explanation at this time.');
    } catch (err) {
