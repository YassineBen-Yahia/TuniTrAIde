...
const StockAnalysis = () => {
  ...
  // AI Recommendation state (placeholder - data will be provided later)
  const [aiRecommendation, setAiRecommendation] = useState({
    action: 'hold', // 'buy', 'sell', 'hold'
    confidence: 72,
    reasoning: 'Based on current market conditions and technical analysis, maintaining position is recommended.',
  });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="panel p-6">
            ...
            {selectedSymbol ? (
              <div className={`rounded-xl border p-6 ${getRecommendationBg(aiRecommendation.action)}`}>
                <div className="flex items-center justify-between mb-4">
                  {getRecommendationIcon(aiRecommendation.action)}
                  <div className="text-right">
                    <div className={`text-2xl font-bold uppercase ${getRecommendationColor(aiRecommendation.action)}`}>
                ...
                <div className="mb-4">
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-slate-200">{aiRecommendation.confidence}%</span>
                  </div>
                  <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                    <div 
                      className={`h-full rounded-full transition-all duration-500 ${
                        aiRecommendation.action === 'buy' ? 'bg-emerald-500' :
                        aiRecommendation.action === 'sell' ? 'bg-rose-500' : 'bg-amber-500'
                      }`}
              ...
              <div className="flex items-center justify-center h-48 text-slate-500 text-center">
                <div>
                  ...
                  <p>Select a stock to see AI recommendation</p>
                </div>
              </div>
            )}
          </div>
